  ...
  try {
    await platformPrisma.$transaction(async (tx) => {
      ...
      await tx.tenantSubscription.create({
        data: { tenantId: tenant.id, planId: data.planId, status: 'TRIAL', trialEndsAt: new Date(Date.now() + 14 * 86400_000) },
      });
      await tx.systemSetting.create({
        data: { tenantId: tenant.id, clubName: data.name, topbarName: data.name, splashLogo: data.name, splashName: data.name, adminUser: '', adminPass: '' },
      });
      const owner = await tx.user.create({
        data: { tenantId: tenant.id, name: data.ownerName, email: data.ownerEmail.toLowerCase(), password, role: 'ADMIN', isActive: true },
      });
    ...
    console.error('Tenant onboarding failed', error instanceof Error ? error.message : 'unknown');
    throw new Error('No se pudo crear el club. Revisá que dominio y correo no estén repetidos.');
  }
}
...
export async function setTenantStatus(formData: FormData) {
  ...
  await platformPrisma.$transaction([
    platformPrisma.tenant.update({ where: { id: tenantId }, data: { status, archivedAt: status === 'ARCHIVED' ? new Date() : null } }),
    platformPrisma.platformAuditLog.create({ data: { actorId: actor.userId, tenantId, action: `TENANT_${status}`, entityType: 'Tenant', entityId: tenantId } }),
    platformPrisma.adminSession.updateMany({ where: { tenantId, revokedAt: null }, data: { revokedAt: new Date() } }),
    platformPrisma.userSession.updateMany({ where: { tenantId, revokedAt: null }, data: { revokedAt: new Date() } }),
  ]);
  clearTenantResolutionCache();
  revalidatePath('/platform');
}
...
export async function setTenantPlan(formData: FormData) {
  ...
  await platformPrisma.$transaction(async (tx) => {
    await tx.tenantSubscription.updateMany({ where: { tenantId, status: { in: ['TRIAL', 'ACTIVE'] } }, data: { status: 'CANCELED', canceledAt: new Date() } });
    await tx.tenantSubscription.create({ data: { tenantId, planId, status: 'ACTIVE', currentPeriodStart: new Date(), currentPeriodEnd: periodEnd } });
    await tx.platformAuditLog.create({ data: { actorId: actor.userId, tenantId, action: 'SUBSCRIPTION_CHANGED', entityType: 'Plan', entityId: planId } });
  });
  clearFeatureCache();
  revalidatePath('/platform');
}
...
export async function setFeatureOverride(formData: FormData) {
  const actor = await requirePlatformAdmin();
  const tenantId = z.string().uuid().parse(formData.get('tenantId'));
  const key = z.enum(FEATURE_KEYS).parse(formData.get('key'));
  const enabled = formData.get('enabled') === 'true';
  await platformPrisma.$transaction([
    platformPrisma.tenantFeatureOverride.upsert({
      where: { tenantId_key: { tenantId, key } },
      create: { tenantId, key, enabled, reason: 'SuperAdmin' },
      update: { enabled, reason: 'SuperAdmin' },
    }),
    platformPrisma.platformAuditLog.create({ data: { actorId: actor.userId, tenantId, action: enabled ? 'FEATURE_ENABLED' : 'FEATURE_DISABLED', entityType: 'Feature', entityId: key } }),
  ]);
  clearFeatureCache();
  revalidatePath('/platform');
}
...
export async function savePlan(formData: FormData) {
  ...
  await platformPrisma.$transaction(async (tx) => {
    const plan = await tx.plan.upsert({
      ...
      create: { code: parsed.code, name: parsed.name, price: parsed.price, currency: 'ARS', isActive: true },
      update: { name: parsed.name, price: parsed.price, isActive: true },
    });
    await tx.planFeature.deleteMany({ where: { planId: plan.id } });
    if (enabledFeatures.length) {
      await tx.planFeature.createMany({ data: enabledFeatures.map(key => ({ planId: plan.id, key, enabled: true })) });
    }
    await tx.platformAuditLog.create({ data: { actorId: actor.userId, action: 'PLAN_SAVED', entityType: 'Plan', entityId: plan.id, metadata: { code: parsed.code } } });
  });
  clearFeatureCache();
  revalidatePath('/platform');
}
...
export async function addTenantDomain(formData: FormData) {
  ...
  await platformPrisma.$transaction(async tx => {
    ...
    await tx.platformAuditLog.create({ data: { actorId: actor.userId, tenantId, action: 'DOMAIN_ADDED', entityType: 'TenantDomain', entityId: domain.id, metadata: { hostname, autoVerified } } });
  });
  clearTenantResolutionCache();
  revalidatePath('/platform');
}
...
export async function verifyTenantDomain(formData: FormData) {
  ...
  const domain = await platformPrisma.tenantDomain.update({ where: { id: domainId }, data: { verifiedAt: new Date() } });
  await platformPrisma.platformAuditLog.create({ data: { actorId: actor.userId, tenantId: domain.tenantId, action: 'DOMAIN_VERIFIED', entityType: 'TenantDomain', entityId: domain.id, metadata: { hostname: domain.hostname } } });
  clearTenantResolutionCache();
  revalidatePath('/platform');
}
